...
const OrderPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      setLoading(true);
      const resp = await axiosRequest<OrderDetail>(
        "GET",
        `/api/orders/${id}/`
      );
      setLoading(false);
    ...
    <div className="max-w-4xl mx-auto p-4 space-y-6 w-full">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center">
        ...
        <div className="space-y-1 text-gray-600">
          ...
          <p>Created: <span className="font-medium text-gray-800">{new Date(order.created_at).toLocaleString()}</span></p>
          <p>Updated: <span className="font-medium text-gray-800">{new Date(order.updated_at).toLocaleString()}</span></p>
        </div>
      </div>

      <div className="flex justify-between items-center mb-4">
        <div className="flex gap-2">
          <p className="text-gray-800 font-medium">{order.user.first_name} {order.user.last_name}</p>
        ...
        {order.items.map((item) => (
          ...
          >
            ...
            <div className="flex items-baseline space-x-4 min-w-60 justify-end">
              <span className="text-gray-800">{item.quantity} × {item.price} Kč</span>
      ...
      <div className="flex justify-end">
        <span className="text-xl font-semibold">
          Total: {order.full_price.toFixed(2)} Kč
        </span>
      </div>
    </div>
  );
