...
import ModalEditProduct, { ProductInput } from "@/components/ModalProduct";
...
const ProductPage = () => {
  ...
  const fetchProduct = async () => {
    setLoading(true);
    const resp = await axiosRequest<ProductType>(
      "GET",
      `/api/products/${id}/`
    );
    setLoading(false);
      ...
      <div className="mt-6 flex flex-col md:flex-row md:justify-between gap-8">
        ...
        <div className="flex-shrink-0 flex flex-col items-start md:items-end space-y-4">
          ...
          <div className="flex gap-2">
            ...
            {product.is_low && (
              <span className="inline-block bg-red-500 text-white px-2 py-0.5 rounded">
                !
              </span>
            )}
          </div>
            ...
            {role === "admin" && (
              <>
                <Button
                  ...
                  onClick={() => showModal(<GeneralModal text={`Do you really want to delete [${product.id}] product?`} actionOnClick={deleteProduct}/>)}
                >
                  Delete
                </Button>
                <Button
                  color="primary"
                  onClick={() => showModal(<ModalEditProduct product={product} updateProduct={updateProduct} />)}
                >
                  Edit
                </Button>
              </>
          ...
          <span className="text-gray-500 mr-auto">
            {cart[product.id] ? `In your cart: ${cart[product.id]}` : ""}
          </span>
        </div>
      </div>
    </div>
